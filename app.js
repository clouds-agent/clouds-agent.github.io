...
async function applyFilter() {
    ...
    const matchMode = document.getElementById('filter-match-mode').value;
    const starred = document.getElementById('filter-starred').value;
    const model = document.getElementById('filter-model').value;
    const groupByPrompt = document.getElementById('filter-group-by-prompt').checked;
    
    const resultEl = document.getElementById('filter-result');
    const groupsEl = document.getElementById('filter-groups');
    const groupInfo = document.getElementById('group-info');
    
    resultEl.innerHTML = '⏳ 正在加载图库...';
    groupsEl.innerHTML = '';
    ...
    try {
        ...
        const token = getToken();
        if (!token) {
            resultEl.innerHTML = '❌ 请先登录';
            return;
        }
        
        const res = await fetch('https://api.talesofai.cn/v1/artifact/list?page_index=0&page_size=100&modality=PICTURE', {
            headers: {
                'x-token': token,
                'x-platform': 'nieta-app/web'
            }
        ...
        const data = await res.json();
        if (!data.list || data.list.length === 0) {
            resultEl.innerHTML = '❌ 没有找到图片';
            return;
        }
        
        resultEl.innerHTML = `⏳ 正在分析 ${data.list.length} 张图片的词条...`;
        
        // 获取每张图片的详细信息
        const imageDetails = [];
        for (let i = 0; i < data.list.length; i++) {
            const item = data.list[i];
            const detailRes = await fetch(`https://api.talesofai.cn/v1/artifact/artifact-detail?uuids=${item.uuid}&is_brief_only=false`, {
                headers: {
                    'x-token': token,
                    'x-platform': 'nieta-app/web'
                }
            ...
            if (detailData && detailData[0]) {
                imageDetails.push(detailData[0]);
            }
            
            // 每 5 张更新一次进度
            if ((i + 1) % 5 === 0) {
                resultEl.innerHTML = `⏳ 正在分析 ${i + 1}/${data.list.length} 张图片...`;
            }
        }
        
        // 应用筛选条件
        filteredImages = imageDetails.filter(img => {
            // 收藏状态筛选
            if (starred !== 'all') {
                if (starred === 'true' && !img.is_starred) return false;
                if (starred === 'false' && img.is_starred) return false;
            }
            
            // 模型筛选
            if (model !== 'all') {
                if (img.input?.context_model_series !== model) return false;
            }
            
            // 关键词筛选
            if (keywords) {
                const keywordList = keywords.split(/[,,]/).map(k => k.trim()).filter(k => k);
                const prompts = img.input?.rawPrompt || [];
                const promptText = prompts.map(p => p.value || '').join(' ');
                
                if (matchMode === 'any') {
                    // 包含任一关键词
                    const hasAny = keywordList.some(kw => kmpSearch(promptText, kw));
                    if (!hasAny) return false;
                } else {
                    // 包含全部关键词
                    const hasAll = keywordList.every(kw => kmpSearch(promptText, kw));
        ...
        resultEl.innerHTML = `✅ 找到 ${filteredImages.length} 张符合条件的图片`;
        
        // 词条归类
        if (groupByPrompt && filteredImages.length > 0) {
            groupImagesByPrompts(filteredImages);
            groupInfo.style.display = 'inline';
            groupInfo.textContent = `已归类为 ${imageGroups.length} 组`;
        } else {
            groupsEl.innerHTML = '';
            imageGroups = [];
        }
        ...
        resultEl.innerHTML = `❌ 筛选失败：${e.message}`;
    }
}

function groupImagesByPrompts(images) {
    imageGroups = [];
    const used = new Set();
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        const baseImg = images[i];
        const basePrompts = (baseImg.input?.rawPrompt || []).filter(p => p.type === 'freetext').map(p => p.value);
        const group = {
        ...
        for (let j = i + 1; j < images.length; j++) {
            ...
            const otherPrompts = (otherImg.input?.rawPrompt || []).filter(p => p.type === 'freetext').map(p => p.value);
            
            // 检查词条是否相同
            let isSame = true;
            if (basePrompts.length !== otherPrompts.length) {
                isSame = false;
            } else {
                for (let k = 0; k < basePrompts.length; k++) {
                    if (!kmpSearch(otherPrompts.join(' '), basePrompts[k])) {
                        isSame = false;
                        break;
                    }
                }
    ...
    // 按组大小排序
    imageGroups.sort((a, b) => b.images.length - a.images.length);
...
function renderGroups() {
    ...
    imageGroups.forEach((group, index) => {
        ...
        const commonPrompts = group.basePrompts.slice(0, 5).join(', ');
        const morePrompts = group.basePrompts.length > 5 ? `... 等${group.basePrompts.length}个词条` : '';
        
        groupEl.innerHTML = `
            <div class="filter-group-header" onclick="toggleGroup(${index})">
                <span class="filter-group-title">📦 第${index + 1}组 - 共同词条：${commonPrompts}${morePrompts}</span>
                <span class="filter-group-count">${group.images.length}张</span>
            </div>
            <div class="filter-group-prompts">
                ${group.basePrompts.map(p => `<span class="prompt-tag">${escapeHtml(p)}</span>`).join('')}
            </div>
            <div class="filter-group-images" id="group-images-${index}" style="display:none">
                ${group.images.map(img => `
                    <div class="filter-group-image" onclick="copyImageUrl('${img.url}')">
                        <img src="${img.url}" alt="${img.uuid}" loading="lazy" />
                        <div class="image-status ${img.status === 'SUCCESS' ? 'success' : 'failure'}">
                            ${img.status === 'SUCCESS' ? '✓' : '✗'}
                        </div>
                    </div>
                `).join('')}
...
function toggleGroup(index) {
    const imagesEl = document.getElementById(`group-images-${index}`);
    if (imagesEl) {
        imagesEl.style.display = imagesEl.style.display === 'none' ? 'grid' : 'none';
    }
}

function copyImageUrl(url) {
    navigator.clipboard.writeText(url).then(() => {
        showToast('已复制 URL，点击打开→', url);
    });
}
...
function clearFilter() {
    ...
    document.getElementById('filter-match-mode').value = 'any';
    document.getElementById('filter-starred').value = 'all';
    document.getElementById('filter-model').value = 'all';
    document.getElementById('filter-group-by-prompt').checked = false;
